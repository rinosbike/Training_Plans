...
export default function Sync() {
  ...
  async function uploadManual() {
    ...
    try {
      ...
      const { data } = await api.post('/api/sync/manual/upload', form)
      qc.invalidateQueries(['sync-status'])
      qc.invalidateQueries({ predicate: q => q.queryKey[0] === 'plan-days' })
      toast.success(t('manualUpload.success', { filename: uploadFile.name, date: data.log_date, sport: data.sport }))
      setUploadFile(null)
      setUploadSport('')
      setUploadDate('')
    } catch (e) {
