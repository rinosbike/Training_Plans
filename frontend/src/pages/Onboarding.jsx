  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-primary-600 px-4 pt-12 pb-6">
        ...
        <h1 className="text-white text-xl font-bold">{t(`steps.${step}`)}</h1>
        <p className="text-primary-200 text-sm">{t('stepOf', { step, total: 3 })}</p>
      </div>

      <div className="flex-1 p-4 space-y-4 overflow-y-auto">
        {step === 1 && (
          <>
            <Field label={t('fields.weight')} type="number" value={profile.weight_kg} onChange={v => setProfile(p => ({...p, weight_kg: v}))} placeholder="70" />
            <Field label={t('fields.height')} type="number" value={profile.height_cm} onChange={v => setProfile(p => ({...p, height_cm: v}))} placeholder="175" />
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{t('fields.fitnessLevel')}</label>
            ...
            <Field label={t('fields.weeklyHours')} type="number" value={profile.current_weekly_hours} onChange={v => setProfile(p => ({...p, current_weekly_hours: v}))} placeholder="5" />
          </>
        )}
          ...
          <div className="grid grid-cols-2 gap-3">
            {GOALS.map(g => (
              <button key={g.type} onClick={() => { setGoal(p => ({...p, goal_type: g.type, goal_name: g.label})); setStep(3) }}
                ...
                <p className="font-semibold text-gray-900 text-sm">{t(`goals.${g.type}.label`, g.label)}</p>
                <p className="text-xs text-gray-500">{t(`goals.${g.type}.desc`, g.desc)}</p>
              </button>
            ))}
          </div>
        )}
        ...
        {step === 3 && (
          <>
            <Field label={t('goalName')} value={goal.goal_name} onChange={v => setGoal(p => ({...p, goal_name: v}))} placeholder={t('goalNamePlaceholder')} />
            <Field label={t('targetDate')} type="date" value={goal.target_date} onChange={v => setGoal(p => ({...p, target_date: v}))} />
            <Field label={t('eventName')} value={goal.event_name} onChange={v => setGoal(p => ({...p, event_name: v}))} placeholder={t('eventNamePlaceholder')} />
          </>
        )}
      </div>
      ...
      <div className="p-4 bg-white border-t border-gray-100 flex gap-3 safe-bottom">
        ...
        {step === 3 && (
          <button onClick={finish} disabled={!goal.target_date || generatePlan.isPending}
            ...
            {generatePlan.isPending ? t('generating') : t('generate')}
          </button>
        )}
      </div>
    </div>
